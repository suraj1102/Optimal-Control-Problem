...
def sample_inputs(n_sample):
    ...
    """
    x1 = np.random.uniform(-1, 1, size=(int(0.8*n_sample), 1))
    x1_mid = np.random.uniform(-0.8, 0.8, size=(int(0.2*n_sample), 1))
    x2 = np.random.uniform(-1, 1, size=((int(0.8*n_sample), 1)))
    x2_mid = np.random.uniform(-0.8, 0.8, size=(int(0.2*n_sample), 1))
...
def main():
    ...
    # Save model
    torch.save(model.state_dict(), "pinn_value_function_di.pth")
